...
//2. データ表示SQL作成
$sql = 'SELECT * FROM gs_bm_table ORDER BY id DESC LIMIT 100';  //更新順にしたい
...
if ($status==false) {
    ...
    while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {    //fetchで一行ずつもってくる whileでレコード分繰り返し
        ...
        $view .= '<p>'.'本のタイトル'.'<strong>『 '.$result['name'].' 』</strong></p>';
        $view .= '<p>'.$result['comment'].'</p>';
        $view .= '<a href="'.$result['url'].'"><p>'.$result['url'].'</p></a>';
        $view .= '<a href="detail.php?id='.$result['id'].'" class="badge badge-primary">更新</a>';
        $view .= '<a href="delete.php?id='.$result['id'].'" class="badge badge-danger">削除</a>'; //アラートを表示したい
        $view .= '<p align=right>作成日時：'.$result['indate'].'</p>';
        $view .= '</li>';
    }
}
